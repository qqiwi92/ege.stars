...
function SwitchNative({
  ...
  const translateX = useDerivedValue(() => (props.checked ? 27 : 1));
  ...
  return (
    <Animated.View
      ...
      className={cn('h-8 w-[56px] rounded-full', props.disabled && 'opacity-50')}
    >
      <SwitchPrimitives.Root
        className={cn(
          'flex-row h-8 w-[56px] shrink-0 items-center rounded-full border-2 border-transparent',
          props.checked ? 'bg-primary' : 'bg-input',
          className
        )}
        {...props}
